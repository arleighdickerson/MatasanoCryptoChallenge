...
object PlaintextScoring {
  val frequencies: Map[Char, Double] = Map(
    ...
    'u' -> 2.758, 'v' -> 0.978, 'w' -> 2.360, 'x' -> 0.150, 'y' -> 1.974, 'z' -> 0.074).map((kv) => kv._1 -> kv._2 / 100)
  ...
  def byCharacters(string: String) = string.toLowerCase.map((char) => frequencies.getOrElse[Double](char, 0)).sum //derp
